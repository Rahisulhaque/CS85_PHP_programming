...
INSERT INTO storedQuery VALUES (
  ...
  'SELECT agent.name AS \'agent\', operation.name AS \'operation\' FROM agent, operation WHERE agent.operationID = operation.operationID'
);
...
INSERT INTO storedQuery VALUES (
  null,
  'agents on operation 2',
  'SELECT agent.name AS \'agent\', operation.name AS \'operation\' FROM agent, operation WHERE agent.operationID = operation.operationID AND agent.operationID = 2'
);

INSERT INTO storedQuery VALUES (
  null,
  'agents and specialties',
  'SELECT agent.name AS \'agent\', specialty.name AS \'specialty\' FROM agent, specialty, agent_specialty WHERE agent.agentID = agent_specialty.agentID AND specialty.specialtyID = agent_specialty.specialtyID'
);
